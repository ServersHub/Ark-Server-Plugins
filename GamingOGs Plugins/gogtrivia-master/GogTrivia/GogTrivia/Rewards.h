...
namespace GogTrivia
{
	...
	public:
		...
		{
			player_ = controller;
			steam_id_ = ArkApi::GetApiUtils().GetSteamIdFromController(controller);
			player_name_ = ArkApi::GetApiUtils().GetCharacterName(controller);
	...
	private:
		...
		{
			ArkApi::GetApiUtils().SendChatMessageToAll(Platform::Msg("Sender"),
				*Platform::Msg("PlayerWon"), player_name_.ToString(), type);
		}
		...
		void RewardRandom()
		{
			...
			while (iterations--)
			{
				...
				{
					index = Platform::RandNumber(0, static_cast<int>(enabled_rewards.size()) - 1);
				}
		...
		void HandleCommands(const nlohmann::json& j)
		{
			...
			while (iterations--)
			{
				...
				{
					index = Platform::RandNumber(0, static_cast<int>(j["Commands"].size()) - 1);
				}
		...
		void HandleLootBoxes(const nlohmann::json& j)
		{
			...
			while (iterations--)
			{
				...
				{
					index = Platform::RandNumber(0, static_cast<int>(j["Boxes"].size()) - 1);
				}

				const std::string box_name = j["Boxes"][index].get<std::string>();
				GogLootBox::GiveBoxes(steam_id_, box_name, 1, "Bounty Hunter Reward", "");
			}
		}
		...
		void HandleDinos(const nlohmann::json& j)
		{
			...
			while (iterations--)
			{
				...
				{
					index = Platform::RandNumber(0, static_cast<int>(j["Dinos"].size()) - 1);
				}
		...
		void HandleItems(const nlohmann::json& j)
		{
			...
			{
				index = Platform::RandNumber(0, static_cast<int>(j["Items"].size()) - 1);
			}
